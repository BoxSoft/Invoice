...
BRW1::View:Browse    VIEW(States)
                       PROJECT(States:StateCode)
                       PROJECT(States:Name)
                     END
Queue:Browse:1       QUEUE                            !Queue declaration for browse/combo box using ?Browse:1
States:StateCode       LIKE(States:StateCode)         !List box control field - type derived from field
States:Name            LIKE(States:Name)              !List box control field - type derived from field
...
QuickWindow          WINDOW('Select a State'),AT(,,173,203),FONT('MS Sans Serif',8,COLOR:Black),RESIZE,CENTER,ICON('USA1.ICO'), |
  GRAY,IMM,MDI,HLP('~SelectaState'),SYSTEM
                       SHEET,AT(4,1,165,199),USE(?CurrentTab),WIZARD
                         TAB('Tab 1'),USE(?Tab1)
                           STRING(@s2),AT(69,185,18,10),USE(States:StateCode),FONT(,,COLOR:Red,FONT:bold),TRN
                         END
                       END
...
ReturnValue          BYTE,AUTO
  ...
  SELF.FirstField = ?States:StateCode
  ...
  BRW1::Sort0:StepClass.Init(+ScrollSort:AllowAlpha,ScrollBy:Runtime) ! Moveable thumb based upon States:StateCode for sort order 1
  BRW1.AddSortOrder(BRW1::Sort0:StepClass,States:StateCodeKey) ! Add the sort order for States:StateCodeKey for sort order 1
  BRW1.AddLocator(BRW1::Sort0:Locator)                     ! Browse has a locator for sort order 1
  BRW1::Sort0:Locator.Init(?States:StateCode,States:StateCode,1,BRW1) ! Initialize the browse locator using ?States:StateCode using key: States:StateCodeKey , States:StateCode
  BRW1.AddField(States:StateCode,BRW1.Q.States:StateCode)  ! Field States:StateCode is a hot field or requires assignment from browse
  BRW1.AddField(States:Name,BRW1.Q.States:Name)            ! Field States:Name is a hot field or requires assignment from browse
