...
QuickWindow          WINDOW('Update Company'),AT(,,199,121),FONT('MS Sans Serif',8,COLOR:Black),RESIZE,CENTER,GRAY, |
  IMM,HLP('~UpdateCompany'),SYSTEM
                       ...
                       ENTRY(@s20),AT(49,7,84,10),USE(COM:Name),CAP,MSG('Company name')
                       PROMPT('Address:'),AT(8,20),USE(?COM:Address:Prompt)
                       ENTRY(@s35),AT(49,20,137,10),USE(COM:Address),CAP,MSG('First line of company''s address')
                       PROMPT('City:'),AT(8,34,17,10),USE(?COM:City:Prompt)
                       ENTRY(@s25),AT(49,34,104,10),USE(COM:City),CAP,MSG('Company''s city')
                       PROMPT('State:'),AT(8,47),USE(?COM:State:Prompt)
                       ENTRY(@s2),AT(49,47,25,10),USE(COM:State),UPR,MSG('Company''s state')
                       PROMPT('Zipcode:'),AT(8,61),USE(?COM:Zipcode:Prompt)
                       ENTRY(@K#####|-####K),AT(49,61,64,10),USE(COM:Zipcode),MSG('Company''s zipcode'),MSG('Company''s zipcode')
