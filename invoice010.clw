...
Customer:FirstName     LIKE(Customer:FirstName)       !List box control field - type derived from field
Customer:MI            LIKE(Customer:MI)              !List box control field - type derived from field
Customer:LastName      LIKE(Customer:LastName)        !List box control field - type derived from field
Customer:Company       LIKE(Customer:Company)         !List box control field - type derived from field
Customer:StateCode     LIKE(Customer:StateCode)       !List box control field - type derived from field
Customer:ZipCode       LIKE(Customer:ZipCode)         !List box control field - type derived from field
Customer:Address1      LIKE(Customer:Address1)        !Browse hot field - type derived from field
Customer:Address2      LIKE(Customer:Address2)        !Browse hot field - type derived from field
Customer:City          LIKE(Customer:City)            !Browse hot field - type derived from field
GLOT:CusCSZ            LIKE(GLOT:CusCSZ)              !Browse hot field - type derived from global data
Customer:PhoneNumber   LIKE(Customer:PhoneNumber)     !Browse hot field - type derived from field
...
Customer:CustomerID    LIKE(Customer:CustomerID)      !Primary key field - type derived from field
...
QuickWindow          WINDOW('Browse Customers'),AT(,,322,227),FONT('MS Sans Serif',8,COLOR:Black),RESIZE,CENTER, |
  ICON('CUSTOMER.ICO'),GRAY,IMM,MDI,HLP('~BrowseCustomers'),SYSTEM
                       SHEET,AT(3,1,318,223),USE(?CurrentTab)
                         TAB('FullName'),USE(?Tab:2)
                           ...
                           STRING(@s25),AT(92,20,104,10),USE(Customer:LastName),FONT(,,COLOR:Red,FONT:bold),TRN
                         END
                         TAB('Company'),USE(?Tab:3)
                           STRING('Locator: Company'),AT(13,20,73,10),USE(?String12),FONT(,,,FONT:bold),TRN
                           STRING(@s20),AT(90,20,83,10),USE(Customer:Company),FONT(,,COLOR:Red,FONT:bold),TRN
                         END
                         TAB('ZipCode'),USE(?Tab:4)
                           STRING('Locator:  Zipcode'),AT(11,20,71,10),USE(?String13),FONT(,,,FONT:bold),TRN
                           STRING(@K#####|-####KB),AT(87,20,51,10),USE(Customer:ZipCode),FONT(,,COLOR:Red,FONT:bold), |
  TRN,TRN
                         END
                         TAB('State'),USE(?Tab:5)
                           STRING(@s2),AT(73,20,26,10),USE(Customer:StateCode),FONT(,,COLOR:Red,FONT:bold),CENTER,TRN
  ...
  FONT:bold+FONT:underline),LEFT(2)
                         STRING(@s35),AT(10,180,169,9),USE(Customer:Address1),FONT(,,COLOR:Teal,FONT:bold)
                         STRING(@s35),AT(10,188,167,9),USE(Customer:Address2),FONT(,,COLOR:Teal,FONT:bold)
                         ...
                         STRING(@P(###) ###-####PB),AT(73,206,72,10),USE(Customer:PhoneNumber),FONT(,,COLOR:Teal,FONT:bold), |
  CENTER
                       END
...
ReturnValue          BYTE,AUTO
  ...
  BRW1::Sort1:StepClass.Init(+ScrollSort:AllowAlpha,ScrollBy:Runtime) ! Moveable thumb based upon Customer:Company for sort order 1
  BRW1.AddSortOrder(BRW1::Sort1:StepClass,Customer:KeyCompany) ! Add the sort order for Customer:KeyCompany for sort order 1
  BRW1.AddLocator(BRW1::Sort1:Locator)                     ! Browse has a locator for sort order 1
  BRW1::Sort1:Locator.Init(?Customer:Company,Customer:Company,1,BRW1) ! Initialize the browse locator using ?Customer:Company using key: Customer:KeyCompany , Customer:Company
  BRW1::Sort2:StepClass.Init(+ScrollSort:AllowAlpha,ScrollBy:Runtime) ! Moveable thumb based upon Customer:ZipCode for sort order 2
  BRW1.AddSortOrder(BRW1::Sort2:StepClass,Customer:KeyZipCode) ! Add the sort order for Customer:KeyZipCode for sort order 2
  BRW1.AddLocator(BRW1::Sort2:Locator)                     ! Browse has a locator for sort order 2
  BRW1::Sort2:Locator.Init(?Customer:ZipCode,Customer:ZipCode,1,BRW1) ! Initialize the browse locator using ?Customer:ZipCode using key: Customer:KeyZipCode , Customer:ZipCode
  BRW1::Sort3:StepClass.Init(+ScrollSort:AllowAlpha,ScrollBy:Runtime) ! Moveable thumb based upon Customer:StateCode for sort order 3
  BRW1.AddSortOrder(BRW1::Sort3:StepClass,Customer:StateKey) ! Add the sort order for Customer:StateKey for sort order 3
  BRW1.AddLocator(BRW1::Sort3:Locator)                     ! Browse has a locator for sort order 3
  BRW1::Sort3:Locator.Init(?Customer:StateCode,Customer:StateCode,1,BRW1) ! Initialize the browse locator using ?Customer:StateCode using key: Customer:StateKey , Customer:StateCode
  BRW1::Sort0:StepClass.Init(+ScrollSort:AllowAlpha,ScrollBy:Runtime) ! Moveable thumb based upon Customer:LastName for sort order 4
  BRW1.AddSortOrder(BRW1::Sort0:StepClass,Customer:KeyFullName) ! Add the sort order for Customer:KeyFullName for sort order 4
  BRW1.AddLocator(BRW1::Sort0:Locator)                     ! Browse has a locator for sort order 4
  BRW1::Sort0:Locator.Init(?Customer:LastName,Customer:LastName,1,BRW1) ! Initialize the browse locator using ?Customer:LastName using key: Customer:KeyFullName , Customer:LastName
  BRW1.AddField(Customer:FirstName,BRW1.Q.Customer:FirstName) ! Field Customer:FirstName is a hot field or requires assignment from browse
  BRW1.AddField(Customer:MI,BRW1.Q.Customer:MI)            ! Field Customer:MI is a hot field or requires assignment from browse
  BRW1.AddField(Customer:LastName,BRW1.Q.Customer:LastName) ! Field Customer:LastName is a hot field or requires assignment from browse
  BRW1.AddField(Customer:Company,BRW1.Q.Customer:Company)  ! Field Customer:Company is a hot field or requires assignment from browse
  BRW1.AddField(Customer:StateCode,BRW1.Q.Customer:StateCode) ! Field Customer:StateCode is a hot field or requires assignment from browse
  BRW1.AddField(Customer:ZipCode,BRW1.Q.Customer:ZipCode)  ! Field Customer:ZipCode is a hot field or requires assignment from browse
  BRW1.AddField(Customer:Address1,BRW1.Q.Customer:Address1) ! Field Customer:Address1 is a hot field or requires assignment from browse
  BRW1.AddField(Customer:Address2,BRW1.Q.Customer:Address2) ! Field Customer:Address2 is a hot field or requires assignment from browse
  BRW1.AddField(Customer:City,BRW1.Q.Customer:City)        ! Field Customer:City is a hot field or requires assignment from browse
  BRW1.AddField(GLOT:CusCSZ,BRW1.Q.GLOT:CusCSZ)            ! Field GLOT:CusCSZ is a hot field or requires assignment from browse
  BRW1.AddField(Customer:PhoneNumber,BRW1.Q.Customer:PhoneNumber) ! Field Customer:PhoneNumber is a hot field or requires assignment from browse
  ...
  BRW1.AddField(Customer:CustomerID,BRW1.Q.Customer:CustomerID) ! Field Customer:CustomerID is a hot field or requires assignment from browse
  ...
  QBE6.AddItem('UPPER(Customer:LastName)','LastName','@s25',1)
  QBE6.AddItem('UPPER(Customer:Company)','Company','@s25',1)
  QBE6.AddItem('UPPER(Customer:ZipCode)','Zipcode','@s10',1)
  QBE6.AddItem('Customer:StateCode','State','@s2',1)
  BRW1.AskProcedure = 1
